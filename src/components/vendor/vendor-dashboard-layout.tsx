...
import { Sparkles, Home, Briefcase, Calendar, FileText, Bell, User, LogOut, Menu, X, CheckCircle2, ChevronLeft, ChevronRight } from 'lucide-react'
      ...
      <aside className={cn(
        ...
        <div className="h-full flex flex-col">
          ...
          <nav className="flex-1 px-3 py-4 space-y-1 overflow-y-auto">
            {vendorSidebarItems.map((item, idx) => {
              const isActive = pathname === item.href
              const showSection = item.section && (idx === 0 || vendorSidebarItems[idx - 1].section !== item.section)
          ...
          <div className="p-3 border-t border-border/50">
            <div className={cn("flex items-center gap-3 px-3 py-2.5 rounded-lg transition-all duration-300", !sidebarOpen && 'md:justify-center')}>
              ...
              <div className={cn("flex-1 min-w-0 transition-all duration-300", !sidebarOpen && 'md:hidden')}>
                <p className="text-sm font-medium truncate">{session?.user?.name?.split(' ')[0] || 'Vendor'}</p>
          ...
          <div className="px-4 md:px-6 py-3.5 flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <h2 className="text-lg font-semibold tracking-tight">
                {vendorSidebarItems.find(item => item.href === pathname)?.title || 'Dashboard'}
              </h2>
            </div>
